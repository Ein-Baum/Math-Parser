 ...
 * @since ALPHA
 */
public class ArcSinExpression implements Expression {

	private Expression expression;
	
	public ArcSinExpression(Expression expression) {
		this.expression = expression;
	}
	
	@Override
	public float computeFloat() {
		return (float) Math.asin(expression.computeDouble());
	}

	@Override
	public int computeInt() {
		return (int) Math.asin(expression.computeDouble());
	}

	@Override
	public double computeDouble() {
		return Math.asin(expression.computeDouble());
	}
	...
	public Expression simplify() {
		...
		if(expression instanceof NumberExpression) {
			return new NumberExpression(Math.asin(expression.computeDouble()));
		}else {
			return this;
		}
	}
